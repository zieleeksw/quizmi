...
export class CategoryCreatePageComponent {
  ...
  readonly isLoadingCourse = signal(true);
  ...
  submit(): void {
    ...
    this.categoryService
      .createCategory(this.courseId, { name })
      .pipe(
      ...
      .subscribe({
        next: () => {
          void this.router.navigate(['/courses', this.courseId, 'categories']);
        },
        error: (error) => {
          this.serverFieldErrors.set(extractFieldErrors(error));
          this.pushErrorToast(extractApiMessage(error) ?? 'Unable to create this category right now.');
        }
      });
  }
  ...
  private loadCourse(): void {
    ...
    this.courseService
      ...
      .pipe(
        finalize(() => this.isLoadingCourse.set(false)),
        takeUntilDestroyed(this.destroyRef)
      )
      .subscribe({
        next: (course) => {
