...
- [x] add frontend quiz overview page inside course workspace
...
- [x] support question count for `category` quizzes
- [x] replace manual question ordering arrows with drag-and-drop composition
...
- [x] allow category quizzes to define how many matching questions should be pulled
...
- [x] add `/courses/:courseId/quizzes/:quizId` page
...
- category quizzes define both selected categories and a question count limit
...
- quiz overview, quiz editing, and quiz analytics are separate user intents and should not be mixed on one screen
- quiz overview is the default entry screen for a single quiz
- quiz editing lives on a dedicated `/edit` route and owns version history
- quiz attempts and review must render from saved snapshots, not from the current live quiz shape
- quiz sessions should remain bound to the quiz shape they started with, even if a newer quiz version is published later
- advanced statistics can start as a lightweight dedicated screen or placeholder, but they should not sit inside the editor by default
...
- let quiz cards open the quiz overview flow, not the editor directly
- use `/courses/:courseId/quizzes/:quizId` as the main quiz overview with `Play` / `Continue`, `Advanced statistics`, `Edit`, and recent attempts
- keep create and edit in one focused quiz editor with a history column on edit
- keep attempts out of the editor and show them in the quiz overview instead
- keep quiz editor interactions direct: manual quiz composition should support drag-and-drop into the selected list instead of relying on up/down controls
